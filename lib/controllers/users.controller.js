...
class UsersController extends RootController {
  ...
  async createContact(req, res) {
    ...
    Promise.all([
      verifyAccessTokenWithId(id, accessToken),
      ...
      .then(async (results) => {
        const [verifiedAccessToken, existingContact, existingDialog] = results
        ...
        if (verifiedAccessToken) {
          if (existingContact && !existingDialog) {
            ...
            const createdNewDialog = await newDialog.save().lean()
            if (createdNewDialog) {
              res
                .status(201)
