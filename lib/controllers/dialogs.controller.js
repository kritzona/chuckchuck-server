...
class DialogsController extends RootController {
  ...
  async createMessage(req, res) {
    ...
    if (sendedMessage) {
      ...
      io.emit(`updated-last-message:user-${sendedMessage.senderId}`, {
        lastMessage: message,
      })
      io.emit(`updated-last-message:user-${sendedMessage.recipientId}`, {
        lastMessage: message,
      })
  ...
  async sendMessage(userId, contactId, dialogId, content) {
    ...
    const updatedDialog =
      ...
      (await this._model.findByIdAndUpdate(dialogId, {
        $addToSet: { messages: [savedMessage.id] },
      }))
