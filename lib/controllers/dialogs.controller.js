...
class DialogsController extends RootController {
  ...
  async show(req, res) {
    ...
    Promise.all([
      verifyAccessTokenWithId(userId, accessToken),
      this.findUserInDialog(userId, dialogId),
    ])
      .then(async (results) => {
        const [verifiedAccessToken, existUserInDialog] = results

        if (verifiedAccessToken && existUserInDialog) {
          const dialogItem = await this._model.findById(userId).lean()
  ...
  async showMessages(req, res) {
    ...
    Promise.all([
      verifyAccessTokenWithId(userId, accessToken),
      this.findUserInDialog(userId, dialogId),
      this._model.findById(dialogId).lean(),
    ])
      .then(async (results) => {
        const [verifiedAccessToken, existUserInDialog, dialog] = results
